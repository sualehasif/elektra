...
parlay::sequence<Element> CreateElements(size_t n, size_t random_seed = 0) {
  ...
  parlay::parallel_for(0, n, [&](const size_t i) {
...
{
  auto joins = parlay::sequence<std::pair<Element*, Element*>>::
    uninitialized(join_indices.size());
  parlay::parallel_for(0, join_indices.size(), [&](const size_t i) {
    joins[i] = std::make_pair(&elements[join_indices[i][0]], &elements[join_indices[i][1]]);
  });
  return joins;
}
