...
struct UndirectedEdgeElements {
  ...
  sequenceReference::Element* const forward_edge;
  sequenceReference::Element* const backward_edge;
};
...
class DynamicForest {
 public:
  ...
  //
  // Efficiency: logarithmic in the size of the forest.
  void MarkEdge(const UndirectedEdgeR& edge, bool mark);
 ...
 private:
  ...
  std::vector<sequenceReference::Element> edge_elements_;
  std::vector<sequenceReference::Element*> free_edge_elements_;
