...
TEST(DynamicForest, AddEdgeAndDeleteEdgePathGraph) {
  ...
  for (int64_t i = 1; i < kNumVertices; i++) {
    dynamic_forest.AddEdge(UndirectedEdgeR(i - 1, i));
  }
  for (int64_t i = 1; i < kNumVertices; i++) {
    EXPECT_TRUE(dynamic_forest.IsConnected(0, i));
  }

  dynamic_forest.DeleteEdge(UndirectedEdgeR(4, 5));
  ...
  for (int64_t i = 1; i < kNumVertices; i++) {
    if (i != 5) {
      dynamic_forest.DeleteEdge(UndirectedEdgeR(i - 1, i));
    }
  }
...
TEST(DynamicForest, AddEdgeAndDeleteEdgeStarGraph) {
  ...
  DynamicForest dynamic_forest(kNumVertices);
  for (int64_t i = 1; i < kNumVertices; i++) {
    dynamic_forest.AddEdge(UndirectedEdgeR(0, i));
  }
  for (int64_t i = 1; i < kNumVertices; i++) {
    EXPECT_TRUE(dynamic_forest.IsConnected(0, i));
  }

  dynamic_forest.DeleteEdge(UndirectedEdgeR(0, 5));
  for (int64_t i = 0; i < kNumVertices; i++) {
  ...
  for (int64_t i = 1; i < kNumVertices; i++) {
    if (i != 5) {
      dynamic_forest.DeleteEdge(UndirectedEdgeR(0, i));
    }
  }
  for (int64_t i = 0; i < kNumVertices; i++) {
    for (int64_t j = i + 1; j < kNumVertices; j++) {
...
TEST(DynamicForest, Mark) {
  ...
  dynamic_forest.MarkEdge(UndirectedEdgeR(2, 3), true);
  EXPECT_THAT(
      dynamic_forest.GetMarkedEdgeInTree(0),
      Optional(UndirectedEdgeR(2, 3)));

  dynamic_forest.MarkEdge(UndirectedEdgeR(6, 7), true);
  dynamic_forest.DeleteEdge(UndirectedEdgeR(2, 3));
  EXPECT_FALSE(dynamic_forest.GetMarkedEdgeInTree(0).has_value());
  EXPECT_FALSE(dynamic_forest.GetMarkedVertexInTree(0).has_value());
  EXPECT_THAT(
      dynamic_forest.GetMarkedEdgeInTree(9),
      Optional(UndirectedEdgeR(6, 7)));
  EXPECT_THAT(dynamic_forest.GetMarkedVertexInTree(9), Optional(8));

  dynamic_forest.MarkEdge(UndirectedEdgeR(6, 7), false);
  ...
  dynamic_forest.MarkEdge(UndirectedEdgeR(6, 7), true);
  dynamic_forest.AddEdge(UndirectedEdgeR(3, 2));
  EXPECT_THAT(
      dynamic_forest.GetMarkedEdgeInTree(0),
      Optional(UndirectedEdgeR(6, 7)));
