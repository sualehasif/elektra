...
class SpanningTree {
 ...
 public:
  ...
  // resize parents to the number of vertices
  // reserve enough space in parents for all elements
  auto parents = parlay::sequence<uintE>::from_function(
      vertices.size(), [](uintE i) { return i; });

  tree = parlay::sequence<E>::from_function(
      vertices.size(), [&](uintE i) { return empty_edge; });
  ...
  // print the parents for each element in edges if you want
